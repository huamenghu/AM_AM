...
function add() {
    ...
    dlg_Edit.dialog('setTitle', '添加模拟参量');
    dlg_Edit_form.url = "/AnalogParameter/Add?tid=";
}
function edit() {
    ...
    dlg_Edit_form.form('clear');
    dlg_Edit.dialog('open');
    dlg_Edit.dialog('setTitle', '您正在修改模拟参量:' + rows[0]['Name']);
    $.post("/AnalogParameter/FillForm/", { id: rows[0]['GUID'] }, function (result) {
        dlg_Edit_form.form('load', result);
    });
    var TGUID = $("#TGUID").val();
    dlg_Edit_form.url = "/AnalogParameter/update?id=" + rows[0]['GUID'] + "&tid=" + TGUID;
}
function del() {
    ...
    if (arr.length > 0) {
        $.messager.confirm('提示信息', '您确认要删除选中的所有记录吗?', function (data) {
            if (data) {
                $.ajax({
                    url: "/AnalogParameter/delete?id=" + arr.join(','),
                    type: 'post',
                    error: function () {
